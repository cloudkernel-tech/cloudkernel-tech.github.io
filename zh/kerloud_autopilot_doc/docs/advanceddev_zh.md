...
针对Kerloud飞控固件在我们的官方Github库中，它会被我们长期维护并持续和px4固件更新．现在最新支持的版本是V1.13.0，用户可以通过下述命令获得固件源码：
...
请注意不同分支对Kerloud飞控兼容性的差异，当前支持状态是：
...
针对Kerloud Uno飞控的固件可以通过make命令编译：
...
* (iv) 如果用户无法下载完整固件源码，请从我们维护的百度云盘下载（注意默认分支）：
